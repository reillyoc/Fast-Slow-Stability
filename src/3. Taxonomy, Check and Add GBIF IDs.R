...
df_mortality <- read.csv("../Mammalian-Fast-Slow-Stability/Data/Hatton et al 2019/slow_fast_mortality.csv", header = T)
...
species_names <- unique(df_mort_sub$Species)
mort_gbif_ids <- get_ids(species_names, db = "gbif", rows=1, messages = TRUE, searchtype = "scientific", accepted = TRUE)

mort_gbif_ids <- mort_gbif_ids$gbif

#Extract the species names and ID #s
matches <- attr(mort_gbif_ids, "match")
names <- attr(mort_gbif_ids, "names")

# Combine into a data frame
df_mort_gbif_ids <- data.frame(
  gbif_id = mort_gbif_ids,
  Species = names,
  stringsAsFactors = FALSE)

df_mort_gbif_ids <- df_mort_gbif_ids %>% select(gbif_id.ids, Species) %>%
  rename(GBIF_ID = gbif_id.ids)

df_mort_gbif_final <- merge(df_mort_sub, df_mort_gbif_ids, by = "Species")

write.csv(df_mort_gbif_final, "../Mammalian-Fast-Slow-Stability/Data/slow_fast_mortality resolved taxonomy.csv")
